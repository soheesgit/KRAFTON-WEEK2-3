...
def n_queens(n: int) -> int:
    ...
    def place(row):  # 퀸을 둘 행인 row
        if row == n:
            return 1

        count = 0
        for c in range(n):  # 이번에 둘 후보 열 c
            safe = True # 충돌이 일어났는지 check하는 boolean

            for i in range(row):  # 이전에 둔 행 검사
            ...
            if safe:
                cols[row] = c  # 퀸을 내려놓기
                count += place(row + 1)  # 재귀 호출해서 다음 행 검사
